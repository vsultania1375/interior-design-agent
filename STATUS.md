...
- Post-result feedback and regeneration: the composer reopens after a plan is generated, routes feedback through the same guardrail helper as other free-text fields, appends it to the original brief as a follow-up refinement note, and reruns the same live-agent path. Each regeneration appends a new result to the chat log rather than overwriting the previous one; the active preview always reflects the latest result. Demo mode leaves the composer visually present but non-functional, with a "live mode only" message, rather than faking a regeneration.
